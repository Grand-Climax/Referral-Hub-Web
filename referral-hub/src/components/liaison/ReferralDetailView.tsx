...
export function ReferralDetailView({ referral }: ReferralDetailViewProps) {
  ...
  const steps = [
    ...
    { label: "Receiver Acceptance", icon: Building, active: false, completed: false },
    ...
    <div className="p-4 md:p-8 max-w-[1400px] mx-auto space-y-6 bg-slate-50 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 border-b pb-6">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <Badge variant="outline" className="text-blue-600 border-blue-200 bg-blue-50 font-semibold uppercase">
      ...
      <div className="w-full py-6 px-4 bg-card rounded-lg border shadow-sm">
        <div className="flex items-center justify-between relative">
          ...
          <div className="absolute left-[10%] right-[50%] top-6 h-0.5 bg-blue-500 -z-10" />
          {steps.map((step, idx) => {
              ...
              <div key={idx} className="flex flex-col items-center gap-2 bg-card px-2">
                ...
                <span
                  className={`text-xs font-semibold ${
                    step.active || step.completed ? "text-blue-600" : "text-muted-foreground"
                  }`}
                >
                  {step.label}
                </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="shadow-sm border">
            <CardHeader className="flex flex-row items-center gap-2 pb-4">
              ...
              <CardTitle className="text-lg">Referral Reason & Clinical Notes</CardTitle>
            </CardHeader>
        ...
        <div className="space-y-6">
          ...
          {/* Origin Details */}
          <Card className="shadow-sm border bg-blue-50/30 overflow-hidden">
          ...
          <Card className="shadow-sm border">
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center space-x-2">
                <Checkbox id="identity" defaultChecked className="scale-50 data-[state=checked]:bg-blue-600" />
                <label htmlFor="identity" className="text-xs font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                  Patient Identity Verified
                </label>
              </div>
              <div className="flex items-center space-x-2">
                <Checkbox id="history" defaultChecked className="scale-50 data-[state=checked]:bg-blue-600" />
                <label htmlFor="history" className="text-xs font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                  Clinical History Attached
                </label>
              </div>
              <div className="flex items-center space-x-2">
                <Checkbox id="labs" className="scale-50 data-[state=checked]:bg-blue-600" />
                <label htmlFor="labs" className="text-xs font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                  Lab Results Included
                </label>
              </div>
              <div className="flex items-center space-x-2">
                <Checkbox id="insurance" className="scale-50 data-[state=checked]:bg-blue-600" />
                <label htmlFor="insurance" className="text-xs font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
          ...
          <Card className="border-none bg-slate-50/50">
            <CardContent className="p-4 space-y-3">
              <Button size="lg" className="w-full bg-emerald-600 hover:bg-emerald-700 shadow-lg shadow-emerald-600/20 py-6 text-base">
                <CheckCircle2 className="mr-2 h-5 w-5" />
                Approve Referral
              </Button>
              
              <div className="grid grid-cols-2 gap-2">
                <Button variant="ghost" className="text-xs text-slate-600 hover:text-amber-700 hover:bg-amber-50">
                  <Undo2 className="mr-1 h-3 w-3" />
                  Return to Doctor
                </Button>
                <Button variant="ghost" className="text-xs text-slate-600 hover:text-rose-700 hover:bg-rose-50">
            ...
            <CardContent>
              <div className="relative border-l-2 border-muted border-dashed ml-3 space-y-6 pb-2">
                <div className="relative pl-6">
                  <div className="absolute -left-[13px] top-1 h-6 w-6 rounded-full flex items-center justify-center border-4 border-card bg-blue-100">
                ...
                {referral.comments && referral.comments.length > 0 && referral.comments.map((comment: any, index: number) => (
                  <div key={index} className="relative pl-6">
                    ...
                    <div>
                      <p className="text-[10px] text-muted-foreground font-semibold">{new Date(comment.createdAt).toLocaleString()}</p>
                      <p className="font-semibold text-sm text-foreground mt-0.5">Note Added</p>
                      <p className="text-xs text-muted-foreground mt-0.5">{comment.author}</p>
                    </div>
                  </div>
                ))}
              </div>
